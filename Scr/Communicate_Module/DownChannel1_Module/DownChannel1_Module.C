...
//下行通道任务信息结构体
struct DownChanne1_TaskStruct 
{
    unsigned char Status;     //任务状态寄存器 BIT0总开关
    unsigned char TaskID;     //任务号，每个任务均有一个ID号
};
struct DownChanne1_TaskStruct DownChanne1_TaskInfo;
...
{
    return Down1CJ188_SumCheck(SumCheckBuff,SumCheckLent);
}


/******************************************************************************
  函数(模块)名称:unsigned char DownChanne1_ReadReciStatus(void)
  功能:	读接收缓冲区状态寄存器
  输入参数:无         	              		
  输出参数: 缓冲区状态寄存器         	 		   		 
  其它说明:
...
*******************************************************************************/
unsigned char DownChanne1_ReadReciStatus(void)
{
    return USCIA2_ReadRXBuffStatus() ;
}

unsigned char DownChanne1_ReadStatus(void)
{
    return DownChanne1_TaskInfo.Status ;
}

/******************************************************************************
  函数(模块)名称:void DownChanne1_Init(void)
  功能:	下行通道通讯波特率设置函数
  输入参数:无         	              		
  输出参数: 无        	 		   		 
  其它说明:默认使用2400波特率，偶校验，8位数据位，1位停止位
*******************************************************************************/
void DownChanne1_Init(void)
{
    struct CommuniPortInfoStruct USCIA2Info;
    unsigned char Data;
    if( ReadUSCIA2Info(&USCIA2Info) )
    { 
        USCIA2_UARTSet(USCIA2Info.Band,USCIA2Info.Pariy,USCIA2Info.Data,USCIA2Info.Stop);
    }
    else
    {
        USCIA2_UARTSet(2400,2,8,1);
    }
...
/******************************************************************************
  函数(模块)名称:unsigned char DownChanne1_MultiByteSend(unsigned char *SendBuffer,unsigned int BufferLent)
  功能:下行通道多字节发送函数	
  输入参数:字节缓冲指针，长度         	              		
  输出参数: 无         	 		   		 
  其它说明:属 
*******************************************************************************/
unsigned char DownChanne1_MultiByteSend(unsigned char *SendBuffer,unsigned int BufferLent)
{
    return USCIA2_MultiByteTX(SendBuffer,BufferLent);
}



/******************************************************************************
  函数(模块)名称:void RecievProcess(struct Down1FrameStruct *FrameTemp)
  功能:	读计量数据的数据处理
  输入参数:无         	              		
  输出参数:无          	 		   		 
  其它说明:
*******************************************************************************/
void RecievProcess(struct Down1FrameStruct *FrameTemp)
{
...
{
    if( 0 == (DownChanne1_TaskInfo.Status & BIT0) )
    {
        DownChanne1_TaskInfo.Status |= BIT0;
        DownChanne1_TaskInfo.TaskID = 0X00;
        return 1;
    }
    else return 0;
}
...
{
      DownChanne1_TaskInfo.Status &= ~BIT0;
      DownChanne1_TaskInfo.TaskID = 0XFF;
      ...
      USCIA2_FrameAckTime_En(0);
      USCIA2_FrameAckTime_SubEn(0);
      USCIA2_FrameAckTime_TimeCntWrite(0);    //重新初始化任务读数
      USCIA2_FrameAckTime_ReTryCntWrite(0);
...
unsigned char ReadMeasureData_CommeSend(void)
{
    ...
    return CJ188_CommendPackSend_USCIA2(ReadMeasureDataInfo.MeterType,ReadMeasureDataInfo.DeviceListItem.DeviceIDBuff,
                                 0X01,0X03,DataBuffer);
}


/******************************************************************************
  函数(模块)名称:void ReadMeasureData_CJ188_ReceProce(struct Down1FrameStruct *FrameTemp)
  功能:	读计量数据任务的接收188解码
  输入参数:帧结构体指针         	              		
  输出参数:          	 		   		 
...
*******************************************************************************/
unsigned char  CJ188_ReceProce(struct Down1FrameStruct *FrameTemp,unsigned char DeviceType,
                               unsigned char *DeviceIDBuffer)
{
    ...
    if( USCIA2_ReadRXBuffFrame() || (USCIA2_ReadRXBuffStatus()&BufferOV_BIT) )
    {
        if( Down1CJ188_GetFrame( FrameTemp ) )           //从缓冲区中检出一帧数据
        {
            if( Down1CJ188_FrameCheckSum(FrameTemp) ) //帧校验
            {
...
{
    struct Down1FrameStruct FrameTemp;
    if( ReadMeasureDataInfo.Status & MeterReadTask_Bit )//抄表任务开启
    {
        switch( ReadMeasureDataInfo.CurrenStatus )
        {
            case 0x00:  //初始化阶段
              ...
              USCIA2_FrameAckTime_TimeCntWrite(ACKTimeNum);
              USCIA2_FrameAckTime_ReTryCntWrite(3);
            ...
            case 0x01: //命令发送阶段
              if( ReadMeasureData_DeviceListItem() )  //从从机列表中取出一个有效的条目
              {
                  ...
                  if( ReadMeasureData_CommeSend() )   //发送抄表命令
                  {
                      ...
                      USCIA2_FrameAckTime_En(1); //计时任务总开关开，当命令帧发送完后，会马上打开计时任务子开关
                  }
                  else
                  {
            ...
            case 0x02: //接收处理阶段
              if( USCIA2_FrameAckTime_ReTryCntRead() )     //检查尝试次数
              {
                  if( USCIA2_FrameAckTime_TimeCntRead() )  //检查是否超时
                  {
                      ...
                      case 0x00://没有数据
                        break;
                      case 0x01://数据有误
                          USCIA2_FrameAckTime_TimeCntWrite(ACKTimeNum); //重新加载时间读数
                          USCIA2_FrameAckTime_ReTryCntDec();            //次数减1
                          ReadMeasureDataInfo.CurrenStatus = 0X01;          //转到阶段,重新尝试
                        break;
                      case 0x02://数据反射
                          USCIA2_FrameAckTime_SubEn(1);//重新开始计时
                        break;
                      case 0x03://数据正确
                          RecievProcess(&FrameTemp);  //处理数据
                          
                          ReadMeasureDataInfo.MeterListNum++;          //指向下一个表
                          USCIA2_FrameAckTime_TimeCntWrite(ACKTimeNum);    //重新初始化任务读数
                          USCIA2_FrameAckTime_ReTryCntWrite(3);
...
unsigned char SeareCommeSend(void)
{
    ...
    DataBuffer[1] = 0x1f;
    DataBuffer[2] = 0x00;
    return CJ188_CommendPackSend_USCIA2(SeareDeviceInfo.DeviceType,SeareDeviceInfo.CurrenDeviceStarID,0X01,0X03,DataBuffer);
...
unsigned char SeareDevcie_TaskOn(void)
{
    ...
    {
        DownChanne1_TaskInfo.Status |= BIT0;
        DownChanne1_TaskInfo.TaskID = 0X01;
        return 1;
    }
    else return 0;
}
...
{
      DownChanne1_TaskInfo.Status &= ~BIT0;//清主任务标志位及任务ID号
      DownChanne1_TaskInfo.TaskID = 0XFF;
...
void SeareDevcie_Task(void)
{
    ...
    if( SeareDeviceInfo.Status & BIT0 )//查询搜表任务标志位
    {
        ...
        {
            case 0x00:  //初始化阶段
              USCIA2_FrameAckTime_TimeCntWrite(ACKTimeNum);//响应时间
              USCIA2_FrameAckTime_ReTryCntWrite(3);//错误尝试次数
              SeareDeviceInfo.CurrenStatus = 0X01; //转到命令发送阶段
              
            case 0x01:  //命令发送阶段
              if( SeareDeviceInfo.SeareCnt < SeareDeviceInfo.SeareLent  )//查询是否搜索完成
              {
                  USCIA2_RXBuffClearn(); //清接收缓冲区
            ...
            case 0x02://接收处理阶段
              if( USCIA2_FrameAckTime_ReTryCntRead() )     //检查尝试次数
              {
                  ...
                  {   //DownChanne1_TaskInfo.TryTime 次连接超时，则重新下一个
                      USCIA2_FrameAckTime_TimeCntWrite( ACKTimeNum ); //重新加载时间读数
                      USCIA2_FrameAckTime_ReTryCntDec();              //次数减1
                      SeareDeviceInfo.CurrenStatus = 0X01;            //转到阶段,重新尝试
                  }
...
/******************************************************************************
  函数(模块)名称:void DownChanne1_TaskProcess(void)
  功能:	下行通道0任务处理函数
  输入参数:无         	              		
  输出参数:         	 		   		 
  其它说明:下行通道的所有任务均在此任务函数内处理
*******************************************************************************/
void DownChanne1_TaskProcess(void)
{
    ...
    if( DownChanne1_TaskInfo.Status & BIT0 )
    {
        switch( DownChanne1_TaskInfo.TaskID )
        {
        case 0x00: 
...
unsigned char ReadDownChanne1_TaskStatus(void)
{
    return DownChanne1_TaskInfo.Status;
}

unsigned char ReadDownChanne1_TaskID(void)
{
    return DownChanne1_TaskInfo.TaskID;
}
