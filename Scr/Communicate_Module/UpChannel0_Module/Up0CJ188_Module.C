...
#include ".\USCIA3_UART_Module\USCIA3_UART_Module.H" //上行通道层通讯
...
/******************************************************************************
  ...
  其它说明:属于内部函数
*******************************************************************************/
static unsigned char FrameCheckSum(struct Up0FrameStruct *FrameTemp)
{
                  ...
                  unsigned char DataLent,unsigned char *DataBuffer)
{
    ...
    else
    {
        ...
        while(tryTime<50000)
        {
            if ( USCIA3_MultiByteTX(FrameBuffer,15+DataLent) )
            {
                USCIA1_MultiByteTX(FrameBuffer,15+DataLent);
                i = 1;
                break;
            }
            tryTime++;
...
/******************************************************************************
  函数(模块)名称:static unsigned char GetFrame(struct Up0FrameStruct *FrameTemp)
  功能:	从接收缓冲区中检出一帧数据
...
*******************************************************************************/
static unsigned char GetFrame(struct Up0FrameStruct *FrameTemp)
{
    unsigned int  BufferLent;
    unsigned char ReadData;
    //USCIA3_FrameDec();
    BufferLent = USCIA3_ReadRXBuffSize();     //取得当前缓冲区大小
    if(BufferLent >= 13)                      //算上68，一帧数据最少13个字节
    {
        while(BufferLent--)
        { 
            if( USCIA3_ReadRXBuffer(&ReadData) )
            {
                if( 0x68 == ReadData)        //取得缓冲区一个字节数据//帧头判断
                {
                    if(BufferLent > 12) 
                    {
                        ReadData = USCIA3_ReadBufferOffset(9); //取数据域长度字节
                        if( BufferLent >= (10+ReadData+2) )    //缓冲区字节数大于帧数据域长度＋校验字节和帧结束字节
                        {
                            ReadData = USCIA3_ReadBufferOffset( 9+ReadData+1+1 );//取得帧结束符位置 数据域长度＋校验符长度
                            if(0x16 == ReadData)
                            {
                                ReadData = USCIA3_ReadBufferOffset(9);           //取数据域长度字节
                                ...
                                {
                                    USCIA3_ReadRXBuffer( &FrameTemp->FrameBuffer[i+1] );
                                }
                                return 1;
                            }
                            continue;
...
/******************************************************************************
  函数(模块)名称:static unsigned char ReadFun_MeterReadData( struct Up0FrameStruct *FrameTemp )
  功能:	读抄表日命令功能函数
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 需要提供抄表日数据
*******************************************************************************/
static unsigned char ReadFun_MeterReadData( struct Up0FrameStruct *FrameTemp )
{
...
/******************************************************************************
  函数(模块)名称:static unsigned char ReadDataType_Decode(struct Up0FrameStruct *FrameTemp )
  功能:	标准读数据命令解析
  输入参数: 解析的命令帧指针
  输出参数: 成功或失败         	 		   		 
  其它说明:  
*******************************************************************************/
static unsigned char ReadDataType_Decode(struct Up0FrameStruct *FrameTemp )
{
...
/******************************************************************************
  函数(模块)名称:static ReadAddr_Decode(struct Up0FrameStruct *FrameTemp )
  功能:	广播读地址命令功能函数
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 会解析数据标识 0X810A以及是否是广播地址: AA AA AA AA AA AA AA
            需要提供采集器ID
*******************************************************************************/
static unsigned char ReadFun_DeviceAddr(struct Up0FrameStruct *FrameTemp )
{
    unsigned int DataType = ((unsigned int)FrameTemp->FrameBuffer[11])<<8 | ((unsigned int)FrameTemp->FrameBuffer[12]);
...
/******************************************************************************
  函数(模块)名称:static unsigned char WriteFun_DeviceAddr(struct Up0FrameStruct *FrameTemp )
  功能:	写地址命令功能函数
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 会解析数据标识 0X810A以及是否是广播地址: AA AA AA AA AA AA AA
*******************************************************************************/
static unsigned char WriteFun_DeviceAddr(struct Up0FrameStruct *FrameTemp )
{
    unsigned int DataType = ((unsigned int)FrameTemp->FrameBuffer[11])<<8 | ((unsigned int)FrameTemp->FrameBuffer[12]);
...
/******************************************************************************
  函数(模块)名称:static unsigned char WriteFun_MeterReadData( struct Up0FrameStruct *FrameTemp )
  功能:	写抄表日命令功能函数
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 需要提供抄表日数据
*******************************************************************************/
static unsigned char WriteFun_MeterReadData( struct Up0FrameStruct *FrameTemp )
{
    unsigned char DataBuffer[4];    
    if( IsDeviceIDEqu( &FrameTemp->FrameBuffer[2] ) ) //判断表号
    {
...
/******************************************************************************
  函数(模块)名称:static unsigned char WriteFun_StandardData( struct Up0FrameStruct *FrameTemp )
  功能:	写标准时间功能函数
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 
*******************************************************************************/
static unsigned char WriteFun_StandardData( struct Up0FrameStruct *FrameTemp )
{
    unsigned char DataBuffer[3];    
    if( IsDeviceIDEqu( &FrameTemp->FrameBuffer[2] ) ) //判断表号
    {
...
/******************************************************************************
  函数(模块)名称:static unsigned char WriteDataType_Decode(struct Up0FrameStruct *FrameTemp )
  功能:	读数据命令
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明:  
*******************************************************************************/
static unsigned char WriteDataType_Decode(struct Up0FrameStruct *FrameTemp )
{
    unsigned int DataType = ((unsigned int)FrameTemp->FrameBuffer[11])<<8 | ((unsigned int)FrameTemp->FrameBuffer[12]);
    switch( DataType )
    {
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtReadFun_DeviceItem( struct Up0FrameStruct *FrameTemp )//扩展读表号
...
*******************************************************************************/
static unsigned char ExtReadFun_DeviceItem( struct Up0FrameStruct *FrameTemp )//扩展读表号
{
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtReadFun_DeviceCurrenData( struct Up0FrameStruct *FrameTemp )
  ...
  其它说明: 返回数据:DI0 DI1 SER 设备类型 序号 ID共 12个字节数据 
            unsigned char ExtReadDeviceIDRead( unsigned char ListNum,unsigned char *DataBuff )
*******************************************************************************/
static unsigned char ExtReadFun_DeviceCurrenData( struct Up0FrameStruct *FrameTemp )
{
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtReadFun_StandardData( struct Up0FrameStruct *FrameTemp )
  功能:	扩展读标准时间
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 
*******************************************************************************/
static unsigned char ExtReadFun_StandardData( struct Up0FrameStruct *FrameTemp )
{
...
/******************************************************************************
  ...
  其它说明: 
*******************************************************************************/
static unsigned char ExtReadFun_DownBand( struct Up0FrameStruct *FrameTemp )
{
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtReadFun_DeviceTypeInfo( struct Up0FrameStruct *FrameTemp )
  功能:	读设备类型信息
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 
*******************************************************************************/
static unsigned char ExtReadFun_DeviceTypeInfo( struct Up0FrameStruct *FrameTemp )
{
...
/******************************************************************************
  ...
  其它说明: 
*******************************************************************************/
static unsigned char ExtReadFun_TaskInfo( struct Up0FrameStruct *FrameTemp )
{
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtReadFun_CollectorInfo( struct Up0FrameStruct *FrameTemp )
  功能:	读设备类型信息
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 
*******************************************************************************/
static unsigned char ExtReadFun_CollectorInfo( struct Up0FrameStruct *FrameTemp )
{
...
/******************************************************************************
  ...
  其它说明: 
*******************************************************************************/
static unsigned char ExtReadFun_ALLMeterData( struct Up0FrameStruct *FrameTemp )
{
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtReadDataType_Decode( struct Up0FrameStruct *FrameTemp )
  功能:	扩展的读功能解码
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 
*******************************************************************************/
static unsigned char ExtReadDataType_Decode( struct Up0FrameStruct *FrameTemp )
{
    unsigned int DataType = ((unsigned int)FrameTemp->FrameBuffer[11])<<8 | ((unsigned int)FrameTemp->FrameBuffer[12]);
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtWriteFun_DeviceTypeItem( struct Up0FrameStruct *FrameTemp )
  功能:	根据数据标识返回相应的序号及表号
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 需要提供 设备类型 序号 ID共 9个字节数据 
            unsigned char ExtReadDeviceIDRead( unsigned char ListNum,unsigned char *DataBuff )
*******************************************************************************/
static unsigned char ExtWriteFun_DeviceTypeItem( struct Up0FrameStruct *FrameTemp )
{
...
/******************************************************************************
  ...
  其它说明: 需要提供 设备类型 序号 ID共 9个字节数据 
            unsigned char ExtReadDeviceIDRead( unsigned char ListNum,unsigned char *DataBuff )
*******************************************************************************/
static unsigned char ExtWriteFun_DeviceItem( struct Up0FrameStruct *FrameTemp )//扩展读表号
{
    unsigned char DataBuff[12];
    if( IsDeviceIDEqu( &FrameTemp->FrameBuffer[2] ) ) //判断表号
    {
...
/******************************************************************************
  ...
  其它说明: 
*******************************************************************************/
static unsigned char ExtWriteFun_DownBand( struct Up0FrameStruct *FrameTemp )
{
    unsigned char DataBuff[10];
    if( IsDeviceIDEqu( &FrameTemp->FrameBuffer[2] ) ) //判断表号
    {
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtWriteFun_DelDeviceTypeItem( struct Up0FrameStruct *FrameTemp )
  功能:	删除设备类型
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 
*******************************************************************************/
static unsigned char ExtWriteFun_DelDeviceTypeItem( struct Up0FrameStruct *FrameTemp )
{
...
/******************************************************************************
  ...
  其它说明: 
*******************************************************************************/
static unsigned char ExtWriteDataType_Decode( struct Up0FrameStruct *FrameTemp )
{
    unsigned int DataType = ((unsigned int)FrameTemp->FrameBuffer[11])<<8 | ((unsigned int)FrameTemp->FrameBuffer[12]);
    switch( DataType )
    {
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtCTLFun_MBUSON(  struct Up0FrameStruct *FrameTemp )
  功能:	扩展的控制指令解码,打开M-Bus总线
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 
*******************************************************************************/
static unsigned char ExtCTLFun_MBUSON(  struct Up0FrameStruct *FrameTemp ) //扩展的总线打开指令
{
...
/******************************************************************************
  ...
  其它说明: 
*******************************************************************************/
static unsigned char ExtCTLFun_MBUSOFF(  struct Up0FrameStruct *FrameTemp ) //扩展的总线打开指令
{
    if( IsDeviceIDEqu( &FrameTemp->FrameBuffer[2] ) )
    {
        if( FrameTemp->FrameBuffer[9] & BIT6 )
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtCTLFun_ClearALLList(  struct Up0FrameStruct *FrameTemp ) //扩展的列表清除指令
  功能:	扩展的控制指令解码
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 删除第一种设备类型对应的设备列表 
*******************************************************************************/
static unsigned char ExtCTLFun_ClearALLList(  struct Up0FrameStruct *FrameTemp ) //扩展的列表清除指令
{
    unsigned char DataBuff[3];
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtCTLFun_SeareDevcie(  struct Up0FrameStruct *FrameTemp ) //扩展的列表清除指令
  功能:	扩展的控制指令解码
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 
*******************************************************************************/
static unsigned char ExtCTLFun_SeareDevcie(  struct Up0FrameStruct *FrameTemp )
{
    unsigned char DataBuff[4];
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtCTLFun_ReadMeasureData(  struct Up0FrameStruct *FrameTemp ) //扩展的列表清除指令
  功能:	扩展的控制指令解码
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 
*******************************************************************************/
static unsigned char ExtCTLFun_ReadMeasureData(  struct Up0FrameStruct *FrameTemp ) //扩展的
{
    unsigned char DataBuff[4];
...
/******************************************************************************
  函数(模块)名称:static unsigned char ExtCTLDataType_Decode( struct Up0FrameStruct *FrameTemp )
  功能:	扩展的控制指令解码
  输入参数: 
  输出参数: 成功或失败         	 		   		 
  其它说明: 
*******************************************************************************/
static unsigned char ExtCTLDataType_Decode( struct Up0FrameStruct *FrameTemp )
{
    unsigned int DataType = ((unsigned int)FrameTemp->FrameBuffer[11])<<8 | ((unsigned int)FrameTemp->FrameBuffer[12]);
    switch( DataType )
    {
...
/******************************************************************************
  函数(模块)名称:static unsigned char ControlCode_Decode(struct Up0FrameStruct *FrameTemp )
  功能:	188控制码解析函数
  输入参数: 
  输出参数: 成功或失败         	 		   		 
...
*******************************************************************************/
static unsigned char ControlCode_Decode(struct Up0FrameStruct *FrameTemp )
{
...
*******************************************************************************/
static unsigned char CJ188_Decode(struct Up0FrameStruct *FrameTemp )
{
...
*******************************************************************************/
unsigned char Up0CJ188_ReceivProcess(struct Up0FrameStruct *FrameTemp)
{
    if( USCIA3_ReadRXBuffFrame() || (USCIA3_ReadRXBuffStatus()&BIT1) )
    {
        ...
        if(GetFrame(FrameTemp))   //从缓冲区中检出一帧数据
        {
            ...
            {
                USCIA1_MultiByteTX(FrameTemp->FrameBuffer,FrameTemp->FrameLent);
                //USCIA1_ByteSend(0x01);
